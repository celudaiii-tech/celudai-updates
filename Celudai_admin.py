...
try:
    ...
    # Fix para PyInstaller: asignar ruta de androguard.conf cuando se ejecuta como exe
    if getattr(sys, 'frozen', False):
        base_path = sys._MEIPASS
        androconf.CONF_PATH = os.path.join(base_path, 'androguard', 'androguard.conf')

except ImportError:
    messagebox.showerror("Error", "Androguard no está instalado. Ejecuta 'pip install androguard'")
    sys.exit(1)
...
def leer_apk(apk_path):
    try:
        ...
        # Intentar obtener package y version de varias formas
        package = getattr(apk, "get_package", lambda: None)()
        version = getattr(apk, "get_androidversion_code", lambda: None)()

        # fallback usando atributos
        if not package:
            package = getattr(apk, "package", None)
        if not version:
            version = getattr(apk, "version_code", None)

        if not package or not version:
            manifest = apk.get_manifest()
            package = manifest.get("package", package)
            version = manifest.get("versionCode", version)
